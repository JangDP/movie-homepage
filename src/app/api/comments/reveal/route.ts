...
export async function POST(request: Request) {
  ...
  const { data, error } = await callSupabaseRpc<
    Array<{ ok: boolean; message: string; comment_id: string | null; body: string | null }>
  ...
  if (error) {
    return NextResponse.json({ ok: false, message: `비밀글 확인 실패: ${error}` }, { status: 500 });
  }
  ...
  return NextResponse.json({
    ok: true,
    comments: (data ?? []).map((item) => ({ id: item.comment_id, body: item.body })).filter((item) => item.id),
  });
}
